...
test_that("restricted argument to graph.bfs is 0 indexed for some reason", {
  ...
  expect_equal(as.numeric(igraph::graph.bfs(g, 1, restricted = c(1,2), unreachable=F)$order), as.numeric(c(NA,NA,NA)))
  expect_equal(as.numeric(igraph::graph.bfs(g, 1, restricted = c(0,1), unreachable=F)$order), c(1,2,NA))
})
